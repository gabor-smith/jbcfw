...
void ST7920_SPI::drawLineVfast(uint8_t x, uint8_t y0, uint8_t y1, uint8_t col)
{
  ...
  mask = 0x80 >> (x&7);
  switch(col) {
    case 1: for(char y=y0; y<=y1; y++) scr[y*scrWd+x/8] |= mask;   break;
    case 0: for(char y=y0; y<=y1; y++) scr[y*scrWd+x/8] &= ~mask;  break;
    case 2: for(char y=y0; y<=y1; y++) scr[y*scrWd+x/8] ^= mask;   break;
  }
}
// ----------------------------------------------------------------
...
void ST7920_SPI::drawLineVfastD(uint8_t x, uint8_t y0, uint8_t y1, uint8_t col)
{
  ...
  mask = 0x80 >> (x&7);
  switch(col) {
    case 1: for(char y=y0; y<=y1; y+=2) scr[y*scrWd+x/8] |= mask;   break;
    case 0: for(char y=y0; y<=y1; y+=2) scr[y*scrWd+x/8] &= ~mask;  break;
    case 2: for(char y=y0; y<=y1; y+=2) scr[y*scrWd+x/8] ^= mask;   break;
  }
}*/
// ----------------------------------------------------------------
...
// ----------------------------------------------------------------
char ST7920_SPI::charWidth(uint8_t c, bool last)
{
  //c = convertPolish(c);
  if (c < cfont.firstCh || c > cfont.lastCh)
      return c==' ' ?  1 + cfont.xSize/2 : 0;
  if (cfont.xSize > 0) return cfont.xSize;
  char ys8=(cfont.ySize+7)/8;
  char idx = 4 + (c-cfont.firstCh)*(-cfont.xSize*ys8+1);
  char wd = pgm_read_byte(cfont.font + idx);
  char wdL = 0, wdR = 1; // default spacing before and behind char
  /*if((*isNumberFun)(c)) {
...
// ----------------------------------------------------------------
char ST7920_SPI::strWidth(const char *str)
{
  char wd = 0;
...
// ----------------------------------------------------------------
char ST7920_SPI::printChar(char xpos, char ypos, unsigned char c)
{
  if(xpos >= SCR_WD || ypos >= SCR_HT)  return 0;
  char fht8 = (cfont.ySize + 7) / 8, wd, fwd = cfont.xSize;
  if(fwd < 0)  fwd = -fwd;
  ...
  char x,y8,b;
  int cdata = (c - cfont.firstCh) * (fwd*fht8+1) + 4;//stay
  byte d;
  wd = fontbyte(cdata++);
  char wdL = 0, wdR = 1;
  /*if((*isNumberFun)(c)) {
...
// ----------------------------------------------------------------
char ST7920_SPI::printStr(char xpos, char ypos, const char *str)
{
  unsigned char ch;
  int stl, row;
  int wd = strWidth(str);//stay
  //int wd = 0;

  if(xpos==-1) // right = -1
    xpos = SCR_WD - wd;
  else if(xpos<0) // center = -2
    xpos = (SCR_WD - wd) / 2;
  if(xpos<0) xpos = 0; // left

  while(*str) {
    int wd = printChar(xpos,ypos,*str++);//stay
    xpos+=wd;
    /*if(cr && xpos>=SCR_WD) { 
      xpos=0; 
      ypos+=cfont.ySize; 
      if(ypos>SCR_HT) ypos = 0;
    }*/
  }
